...
function deployMetadata(conn, metadataType, developerName, metadataContent) {
  return new Promise((resolve, reject) => {
    ...
    // Adjust fields based on metadata type
    switch (metadataType) {
      case 'ApexClass':
      case 'ApexTrigger':
        // ApexClass and ApexTrigger use the same structure
        break;
      case 'ApexPage':
        metadataObject.Markup = metadataContent;
        delete metadataObject.Body;
        break;
      case 'ApexComponent':
        metadataObject.Markup = metadataContent;
        delete metadataObject.Body;
    ...
    // Deploy the metadata using the Tooling API
    conn.tooling.sobject(metadataType).create(metadataObject, function (err, result) {
      if (err) {
...
(async () => {
  try {
    ...
    const supportedTypes = ['ApexClass', 'ApexTrigger', 'ApexPage', 'ApexComponent'];
