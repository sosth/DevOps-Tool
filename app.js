...
app.post("/retrieve-all-apex-classes", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "All Apex classes retrieved and saved successfully." });
  } catch (error) {
...
app.get("/get-org-info", async (req, res) => {
  if (req.session.token) {
    try {
      const orgIdData = await getOrgIdApi.getOrgId(
        req.session.token,
        req.session.instanceUrl
      );
      const orgInfo = await getOrgIdApi.getOrgDetails(
        req.session.token,
        req.session.instanceUrl,
        orgIdData.Id
      );
...
app.post("/get-apex-classes", async (req, res) => {
  ...
  try {
    const apexClasses = await conn.metadata.list(
      [{ type: "ApexClass" }],
      "39.0"
    );
